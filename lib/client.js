...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function clampText(text, max) {
      ...
      var limit = typeof max === 'number' && max > 0 ? max : MAX_TEXT
      ...
      return el('div', {
        ...
        el('div', { style: { flex: '1 1 auto', minWidth: 0 } },
          ...
          typeof item.summary === 'string' && item.summary.length > 0
            ? el('div', {
              style: { fontSize: 10.5, color: C.textSub, lineHeight: '15px', marginTop: 1, wordBreak: 'break-word' },
            }, clampText(item.summary, 140))
            : null,
    ...
    function Panel(props) {
      ...
      var goalHints = data !== null && Array.isArray(data.goalHints) ? data.goalHints : []
          ...
          el(Section, { title: '长期任务', stat: longStat, extra: null },
            goal !== null
              ...
              : (goalHints.length > 0
                ? el('div', {
                  style: {
                    border: '1px dashed ' + C.warn, borderRadius: 10,
                    padding: '8px 10px', background: C.surfaceSub,
                  },
                },
                  el('div', {
                    style: { fontSize: 11.5, color: C.warn, fontWeight: 700, marginBottom: 4 },
                  }, '疑似长期需求（尚未建立目标）'),
                  goalHints.map(function (hint) {
                    return el('div', {
                      key: hint,
                      style: { fontSize: 11.5, color: C.text, lineHeight: '17px', wordBreak: 'break-word' },
                    }, '· ' + clampText(hint))
                  }),
                  el('div', {
                    style: { fontSize: 10.5, color: C.textTertiary, marginTop: 5 },
                  }, '让会话为其中一条建立长期目标后，这里会显示轮次进度'))
