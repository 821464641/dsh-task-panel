...
| **兜底自动识别** | 当 Agent 从未写过任何 todo 清单时（内置工具说明允许琐碎任务跳过清单），路由从**会话全部** `user/message`（`source.kind === 'user'`）跨轮积累推导任务：剔除确认语/问题/闲聊/纯信息来源，去除「请/帮我」等礼貌前缀、按标题去重，每条请求一条任务，最新一条标记 `in_progress`（见 §4.5） |
| **LLM 智能记录（优先）** | AI 通过 `task_panel_update` 工具写入的清单是短期任务**首数据源**；`readPanelTasks` 读取并严格归一化（title≤300 字 / status 枚举 / summary≤1200 字，违规即拒绝）。存储：`DSH_TASK_PANEL_STATE_DIR`（默认 `DSH_HOME/task-panel-state/<sessionId>.json`，原子写 tmp+rename）。工具为手工构造注册对象（JSON Schema 直写 + execute 内自校验），包保持零依赖（AgentTeams 式内联打包模式不可复制，见 §2.1） |
| **清单过时不遮挡** | 最后一条 todo/write **之后**到达的新用户请求，经 `foldUserTasksAfter` 推导后由 `mergeTodos` 并入清单显示（`lastTodoWriteTime` 为分界；派生项与清单项按标题精确去重；清单无进行中时最新派生项标 in_progress）。Agent 提交新清单后分界前移，派生项自动消失——旧清单永远挡不住新任务（v0.1.7） |
