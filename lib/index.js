...
async function collectState(ctx, sessionId) {
  ...
  // Agent's latest todo list persists across turns; UNACKNOWLEDGED user
  // requests that arrived after that list are merged in, so a stale list
  // can never hide a newly assigned task. Sessions with no todo history
  // fall back to deriving tasks from all user messages.
