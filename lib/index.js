...
async function collectState(ctx, sessionId) {
  ...
  // Preferred source: the agent's todo list (persists across turns until the
  // next replacement). Once the agent EVER wrote a list, the latest one is
  // authoritative — even when empty. Otherwise derive the user's requests
  // from the session's direct-human messages, so the panel proactively shows
  // short-term tasks in sessions where the agent skipped todo_write.
  const agentTodos = foldTodos(session)
  const goal = readGoal(ctx, sessionId)
  return {
    session: sessionId,
    goal,
    todos: hasTodoHistory(session) ? agentTodos : foldUserTasks(session),
  ...
  if (systemPrompt !== undefined) {
    systemPrompt.section({
      ...
      text: [
        ...
        '代理的执行过程与内部机制不计入。面板会跨轮保留已登记的任务清单（新一轮不清空），直到你提交新清单。',
        '',
        '【短期任务 = todo 清单】',
        ...
        '',
        '【长期任务 = 目标（goal）】',
        '7. 当用户明确表达长期目标（如「长期维护」「持续跟踪」「一直做」「以后定期」）时，',
        '   你**必须**建立目标（create_goal）；目标只对应用户明确要求/共同商定的长期任务，',
        '   不要为内部子目标、执行分段或代理自己推进计划创建目标。',
        '8. 一个用户可见的长期任务对应一个目标；',
