...
| **短期任务粒度** | 面板的短期任务=「用户可见任务」：仅包含用户在会话中明确委派或共同商定的任务。该约定由宿主在 apply 时通过系统提示词段（`task-panel:task-convention`，order 116）向每个会话的代理声明（见 §4.4），代理按此粒度维护 todo 清单；代理的内部操作步骤（建目录、备份、安装、验证、清理、汇报等）不单独成条 |
...
面板的两组任务都直接映射到「用户可见任务」：短期=todo 清单，长期=目标。为落实统一的两级定义
（**明确委派/共同商定的任务列入面板；代理的执行步骤与内部执行机制不计入**），
...
- 给出用户侧示例（「将该插件移入 D:\...，并且要有详细的文档」→「移动插件文件位置」「撰写介绍文档」两条）以锚定短期粒度；
- 明确排除项（短期）：创建/检查目录、备份、安装命令、语法检查、验证/预检、清理旧副本、汇总汇报等；
...
5. 断言空 session 参数 → `session: null` 空载荷；
6. 断言未知 session → 200 + 空数据（优雅降级）。
